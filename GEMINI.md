...
如果你在GeminiCLI里使用，把仓库放进项目后，Gemini可以读取这个文件。需要完整规则时，继续读取`AGENTS.md`。需要模板和示例时，读取`human-social-copy/references/patterns.md`。需要更完整的开头钩子模型时，读取`human-social-copy/references/adaptive-hooks.md`。
